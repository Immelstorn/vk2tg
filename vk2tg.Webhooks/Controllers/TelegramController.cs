...
//            msg.AppendLine("Если что - вот две кнопки, можешь воспользоваться ими.");
            msg.AppendLine("Ну что ж, этот день настал.");
            msg.AppendLine("Пару месяцев назад я просил поддержать бота материально, чтобы я мог оплачивать хостинг.");
            msg.AppendLine("Каждый из вас сделал свой выбор.");
            msg.AppendLine("С данного момента бот перестает работать.");
            msg.AppendLine();
            msg.AppendLine("Если захочется его вернуть - ниже есть два способа это сделать. Если наберется сумма достаточная для оплаты хостинга - бот вернется.");
